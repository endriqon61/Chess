...
const Board = ({currentPlayerColor}: Props) => {
  ...
  return (
    <>
      <div>
        {tiles.numbers.map((number: any, index1: number) => {
          return <div className='h-[75px]'>
            {tiles.letters.map((letter: any, index2: number) => {
              ...
              return <span data-tile={`${letter}${number}`} className={`w-[75px] relative text-lg text-center ${(number + index2 - 1) % 2 == 0 ? 'bg-cyan-800 text-white': 'bg-indigo-100 text-black'}  h-full inline-block`}> 
